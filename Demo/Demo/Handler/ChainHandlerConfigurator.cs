...
public static class ChainHandlerConfigurator
{
    ...
    public static IChainHandlerConfigurator<T> Chain<T>(this IServiceCollection services, object? key) where T : class, IChainHandler 
    {
        return new ChainConfiguratorImpl<T>(services, key);
    }

    private class ChainConfiguratorImpl<T>(IServiceCollection services, object? key)
        : IChainHandlerConfigurator<T> where T : class, IChainHandler
    {
        ...
        private void ConfigureType(Type currentType)
        {
            ...
            var ctorParameters = ctor.GetParameters().Select(p =>
            {
                ...
                if (interfaceType.IsAssignableFrom(p.ParameterType))
                {
                    ...
                    else
                    {
                        ...
                        return Expression.Call(typeof(ServiceProviderKeyedServiceExtensions), "GetRequiredKeyedService",
                            [nextType], parameter, Expression.Constant(key));
                    }
                }
            ...
            services.AddKeyedTransient(resolveType, key,
                (IServiceProvider provider, object? key) => { return compiledExpression.Invoke(provider); });
